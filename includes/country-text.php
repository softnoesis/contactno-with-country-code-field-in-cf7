...
add_action( 'wpcf7_init', 'sncpf_add_form_tag_countrytext' );

function sncpf_add_form_tag_countrytext() {
	wpcf7_add_form_tag(
		array( 'countrytext', 'countrytext*'),
		'sncpf_countrytext_form_tag_handler', array( 'name-attr' => true ) );
}

function sncpf_countrytext_form_tag_handler( $tag ) {
	if ( empty( $tag->name ) ) {
		return '';
	}
...
add_filter( 'wpcf7_validate_countrytext', 'sncpf_countrytext_validation_filter', 10, 2 );
add_filter( 'wpcf7_validate_countrytext*', 'sncpf_countrytext_validation_filter', 10, 2 );

function sncpf_countrytext_validation_filter( $result, $tag ) {
	$type = $tag->type;
	$name = $tag->name;
...
add_action( 'wpcf7_admin_init', 'sncpf_add_tag_generator_countrytext', 20 );

function sncpf_add_tag_generator_countrytext() {
	$tag_generator = WPCF7_TagGenerator::get_instance();
	$tag_generator->add( 'countrytext', __( 'country code', 'sn-pcf' ),
		'sncpf_tag_generator_countrytext' );
}

function sncpf_tag_generator_countrytext( $contact_form, $args = '' ) {
	$args = wp_parse_args( $args, array() );
	$type = 'countrytext';

	$description = __( "Generate a form-tag for a country dorp list with flags icon text input field.", 'sn-pcf' );
